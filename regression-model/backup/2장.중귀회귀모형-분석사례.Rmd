...
```{r}
# 설명변수가 적합되는 순서에 따른 분산분석표 - speed+temp
anova(lm(loss ~ speed+temp, data=chemical) )
```

```{r}
# 설명변수가 적합되는 순서에 따른 분산분석표 - temp+speed
anova(lm(loss ~ temp+speed, data=chemical) )
```
...
# 6) 잔차 산점도 : (독립변수, 잔차)
par(mfrow=c(1,2), pty="s")
plot(chemical$speed, chemical.lm$resid, pch=19, ylim = c(-max(chemical.lm$resid)*2, max(chemical.lm$resid)*2))
abline(h=0, lty=2)
for (i in 1:length(chemical$speed))
{
  if(abs(chemical.lm$resid[i]) > 3.7)
  {
    text(chemical$speed[i]+0.8, chemical.lm$resid[i]-0.5, as.character(i))
  }
}

plot(chemical$temp, chemical.lm$resid, pch=19,ylim = c(-max(chemical.lm$resid)*2, max(chemical.lm$resid)*2))
abline(h=0, lty=2)
for (i in 1:length(chemical$temp))
{
  if(abs(chemical.lm$resid[i]) > 3.7)
  {
    text(chemical$temp[i]+0.4, chemical.lm$resid[i]-0.4, as.character(i)) 
  }
}
```
...
```{r}
plot(chemical.lm$fitted, chemical.lm$resid, ylim = c(-max(chemical.lm$resid)*2, max(chemical.lm$resid)*2))
abline(h=0, lty=2)
for (i in 1:length(chemical$temp))
{
