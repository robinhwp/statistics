...
paste0("hat_Y=", round(b_0,5), " + ", round(b_1,5), "X")
```
...
`predict`함수를 이용한 $\hat{Y}와 \hat{Y}_{new}$의 $100(1-\alpha)$% 신뢰구간구간
```{r}
X=seq(3.5,14.5,0.2)
...
주어진 X값에서 새로운 예측값 $Y_{new}$의 신뢰구간 
$\hat{Y}_{new} \pm t(n-2;\alpha/2) \sqrt{ MSE[1 + {1\over n} + {(X-\bar{X})^2 \over S_{XX}}]}$ 
```{r}
