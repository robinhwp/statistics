...
\end{matrix}$$
여기서 $beta_0^*$는 $\beta_0+\beta_1\bar{X}$를 대체한 것이다. 이 모형은 설명변수로서 $X_i$대신에 ($X_i-\bar{X}$를 사용하는 경우이다.)

## 회귀선의 추정

**최소제곱법(method of least squares)**\
표본자료(sample data)로부터 추정하여 얻은 직선을 $\hat{Y}=b_0+b_1X$라고 하자. 이와같은 직선을 추정된 회귀직선, 또는 간단히 회귀선이라고 한다.\
...
Coefficients:Estimate 절편(Intercept)값과 X값으로 만들어 진다.
...
**결정계수**\
summary(lm) 결과에서 \
*Multiple R-squared:* 값에 해당 market에서는 Multiple R-squared:  0.9369\
